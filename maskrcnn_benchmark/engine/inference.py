...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None, store_union=False):
    model.eval()
    results_dict = {}
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                if store_union:
                    output, rec_all_union_fea, counts, num_sample = model(images.to(device), targets)
                else:
    ...
    if store_union:
        nonzero_id = counts.nonzero()[:, 0]
        save_vec = torch.normal(0, 1, (51, 4096)) # num_class, feature_dim
        save_vec[nonzero_id] = rec_all_union_fea[nonzero_id] / counts[nonzero_id]
        torch.save(save_vec, './output/relation_baseline/vis_protos.pt')
        print('visual prototypes already saved, process stop.')
        return 0
...
def inference(
    ...
    if load_prediction_from_cache:
        ...
        predictions = compute_on_dataset(model, data_loader, device, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER, timer=inference_timer, store_union=cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION)
