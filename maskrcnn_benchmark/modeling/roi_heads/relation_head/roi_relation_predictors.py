...
from maskrcnn_benchmark.modeling.roi_heads.relation_head.VAE_model import VAE_MODEL, VAE_loss
...
class PrototypeEmbeddingNetwork(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.obj_embed = nn.Embedding(self.num_obj_cls, self.embed_dim) # 151,300
        self.rel_embed = nn.Embedding(self.num_rel_cls, self.embed_dim) # 151,300
        with torch.no_grad():
        ...
        self.logit_wt2 = nn.Parameter(torch.rand(1, self.num_rel_cls)).cuda()
        self.logit_wt3 = nn.Parameter(torch.rand(1, self.num_rel_cls)).cuda()
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        entity_rep = self.post_emb(roi_features)   # fc using the roi features obtained from the faster rcnn # 4096→4096
        ...
        for pair_idx, sub_rep, obj_rep, entity_pred, entity_embed, proposal in zip(rel_pair_idxs, sub_reps, obj_reps, entity_preds, entity_embeds, proposals):
            ...
            s_embed = self.W_sub(entity_embed[pair_idx[:, 0]])  #  Ws x ts MLP
            o_embed = self.W_obj(entity_embed[pair_idx[:, 1]])  #  Wo x to MLP

            sem_sub = self.vis2sem(sub_rep[pair_idx[:, 0]])  # MLP h(xs)
            sem_obj = self.vis2sem(obj_rep[pair_idx[:, 1]])  # MLP h(xo)
        ...
        rel_dists = self.pred_classifier(rel_rep)
        rel_dists = (rel_dists - rel_dists.mean(dim=1).reshape(-1, 1)) / rel_dists.std(dim=1).reshape(-1, 1)
        rel_dists1 = torch.zeros_like(rel_dists)
        rel_dists2 = torch.zeros_like(rel_dists)
        rel_dists3 = torch.zeros_like(rel_dists)
