...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        """
        if self.training or self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
        ...
        if self.use_union_box:
            if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
                union_features, rec_all_union_fea, counts, num_sample = self.union_feature_extractor(features, proposals, rel_labels, rel_pair_idxs)
            else:
        ...
        if not self.training:
            ...
            if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
                return roi_features, result, {}, rec_all_union_fea, counts, num_sample
