...
class RelationFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        # only when STORE_UNION == True, for visual prototype initialization
        self.rec_all_union_fea = torch.zeros(51, 4096) # sum of feature of each class
        self.counts = torch.zeros(51, 1) # number of each class
        self.num_sample = 0
    ...
    def forward(self, x, proposals, rel_labels, rel_pair_idxs=None):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION: # only when store_union==True
            rel_labels = torch.cat(rel_labels)
            fg_ids = rel_labels.nonzero().view(-1)
            fg_labels = rel_labels[fg_ids]
            fg_union_features = union_features[fg_ids]

            # only foreground union_feature
            for i, fg_label in enumerate(fg_labels):
                self.num_sample += 1
                uf = fg_union_features[i, :]
                self.counts[fg_label] += 1
                self.rec_all_union_fea[fg_label] += uf.cpu()
            return union_features, self.rec_all_union_fea, self.counts, self.num_sample
