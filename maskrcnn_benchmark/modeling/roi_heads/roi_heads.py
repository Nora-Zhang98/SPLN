    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
                x, detections, loss_relation, rec_all_union_fea, counts, num_sample = self.relation(features, detections, targets, logger)
            else:
                x, detections, loss_relation = self.relation(features, detections, targets, logger)
            losses.update(loss_relation)

        if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
            return x, detections, losses, rec_all_union_fea, counts, num_sample
