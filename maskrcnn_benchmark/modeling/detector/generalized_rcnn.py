    ...
    def forward(self, images, targets=None, logger=None):
        ...
        if self.roi_heads:
            if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
                x, result, detector_losses, rec_all_union_fea, counts, num_sample = self.roi_heads(features, proposals, targets, logger)
            else:
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
            return result, rec_all_union_fea, counts, num_sample
        return result
