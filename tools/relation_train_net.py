...
def train(cfg, local_rank, distributed, logger):
    ...
    if checkpointer.has_checkpoint():
        extra_checkpoint_data = checkpointer.load(cfg.MODEL.PRETRAINED_DETECTOR_CKPT, 
                                       update_schedule=cfg.SOLVER.UPDATE_SCHEDULE_DURING_LOAD, with_optim=True)
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
        return model
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION:
        logger.info("Start store foreground union features")
    else:
...
def run_test(cfg, model, distributed, logger):
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.STORE_UNION: # only for visual prototype initialization
